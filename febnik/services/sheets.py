...
def feedback_sheet_tab_title(settings: Settings, day: int) -> str:
    p = (settings.sheet_feedback_tab_prefix or "ОС день").strip()
    return f"{p} {day}"


def export_feedback_day_to_sheet(
    settings: Settings,
    day: int,
    rows: list[tuple[datetime | None, str, str, str, str | None]],
) -> str | None:
    """
    Перезаписывает одну вкладку книги: заголовки = названия вопросов + метаданные.
    rows: (created_at, full_name, email, student_ticket, answers_json).
    Возвращает текст ошибки или None при успехе.
    """
    import gspread

    from febnik.survey_content import feedback_answer_cells_for_row, feedback_sheet_header_row

    client = _client(settings)
    if not client:
        return "Google не настроен: проверьте GOOGLE_CREDENTIALS_PATH, GOOGLE_SPREADSHEET_ID и пакеты gspread/google-auth."

    header = feedback_sheet_header_row(day)
    if not header:
        return f"Анкета для дня {day} не описана в коде."

    title = feedback_sheet_tab_title(settings, day)
    try:
        sh = client.open_by_key(settings.google_spreadsheet_id)
        try:
            ws = sh.worksheet(title)
            ...
            ws = sh.add_worksheet(
                title=title,
                rows=max(500, len(rows) + 8),
                cols=max(12, len(header) + 2),
            )
        ws.append_row(header)
        for created_at, full_name, email, student_ticket, answers_json in rows:
            ca = created_at.isoformat() if created_at else ""
            ans_cells = feedback_answer_cells_for_row(day, answers_json)
            ws.append_row(
                [
                    ca,
                    full_name,
        ...
        logger.exception("export_feedback_day_to_sheet day=%s", day)
        return str(e)[:800]
    return None


async def export_feedback_day_to_sheet_async(
    settings: Settings,
    day: int,
    rows: list[tuple[datetime | None, str, str, str, str | None]],
) -> str | None:
    return await asyncio.to_thread(export_feedback_day_to_sheet, settings, day, rows)
